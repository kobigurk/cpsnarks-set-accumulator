...
impl<G: UnknownOrderGroup, T: Hash + Into<Integer>, P: AccumulatorParameters> Clone
    for Accumulator<G, T, P>
{
    fn clone(&self) -> Self {
        Self {
            phantom: PhantomData,
...
pub struct MembershipProof<
    G: UnknownOrderGroup,
    T: Hash + Into<Integer> + Clone,
    P: AccumulatorParameters,
> {
...
impl<G: UnknownOrderGroup, T: Eq + Hash + Into<Integer> + Clone, P: AccumulatorParameters>
    Accumulator<G, T, P>
{
...
impl<G: UnknownOrderGroup, T: Clone + Hash + Into<Integer>, P: AccumulatorParameters>
    Witness<G, T, P>
{
    fn hash_to_prime_if_needed(t: &T) -> Integer {
        if P::should_hash_to_prime() {
    ...
    pub fn compute_individual_witnesses(&self, elems: &[T]) -> Vec<(T, Self)> {
        let hashes = elems
            .iter()
            .map(Self::hash_to_prime_if_needed)
            .collect::<Vec<_>>();
        elems
            .iter()
...
*/
    ...
    fn test_delete_bad_witness<G: UnknownOrderGroup>() {
        ...
        let a_witness = Witness(new_acc::<G, Integer, AccumulatorWithHashToPrime>(&[
            2.into(),
            3.into(),
        ]));
        let b_witness = Witness(new_acc::<G, Integer, AccumulatorWithHashToPrime>(&[
            1.into(),
            3.into(),
        ]));
        acc.delete(&[(1.into(), a_witness), (2.into(), b_witness)])
            .unwrap();
    }
    ...
    fn test_update_membership_witness<G: UnknownOrderGroup>() {
        let acc =
            new_acc::<G, Integer, AccumulatorWithHashToPrime>(&[1.into(), 2.into(), 3.into()]);
        let witness = Witness(new_acc::<G, Integer, AccumulatorWithHashToPrime>(&[
            3.into(),
            4.into(),
        ]));
        let witness_new = acc
    ...
    fn test_update_membership_witness_failure<G: UnknownOrderGroup>() {
        ...
        acc.update_membership_witness(witness, &[1.into()], &[2.into()], &[1.into()])
            .unwrap();
    }
    ...
    fn test_compute_sub_witness<G: UnknownOrderGroup>() {
        ...
        let sub_witness = empty_witness
            ...
            .unwrap();
        let exp_quotient_expected = Witness(new_acc::<G, Integer, AccumulatorWithHashToPrime>(&[
            2.into(),
        ]));
    ...
    fn test_compute_individual_witnesses<G: UnknownOrderGroup>() {
        let acc =
            new_acc::<G, Integer, AccumulatorWithHashToPrime>(&[1.into(), 2.into(), 3.into()]);
        let witness_multiple = Witness(new_acc::<G, Integer, AccumulatorWithHashToPrime>(&[
            1.into()
        ]));
