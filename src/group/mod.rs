...
pub trait UnknownOrderGroup: Group {
    ...
    /// Returns an element of unknown order in the group, possibly random.
    fn unknown_possibly_random_order_elem<R: MutRandState>(rng: &mut R) -> Self::Elem {
        Self::unknown_possibly_random_order_elem_(Self::rep(), rng)
    }
    ...
    /// A group-specific wrapper for `unknown_possibly_random_order_elem`.
...
pub trait ElemToBytes: Group {
    /// Returns a group element from an initial value.
    fn elem_to_bytes(val: &Self::Elem) -> Vec<u8>;
}
